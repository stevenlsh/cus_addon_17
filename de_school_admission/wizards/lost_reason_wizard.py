...
class LeadLostWizard(models.TransientModel):
    _name = 'oe.admission.lost.wizard'
    _description = 'Get Lost Reason'

    lost_reason_id = fields.Many2one('oe.admission.lost.reason', 'Lost Reason')
    ...
    def action_lost_reason_apply(self):
        self.ensure_one()
        leads = self.env['oe.admission'].browse(self.env.context.get('active_ids'))

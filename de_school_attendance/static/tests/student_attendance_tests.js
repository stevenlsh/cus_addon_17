odoo.define('de_school_attendance.tests', function (require) {
"use strict";

var testUtils = require('web.test_utils');
var core = require('web.core');

var MyAttendances = require('de_school_attendance.my_attendances');
var KioskMode = require('de_school_attendance.kiosk_mode');
var GreetingMessage = require('de_school_attendance.greeting_message');


QUnit.module('Student Attendance', {
    beforeEach: function () {
        this.data = {
            'res.partner': {
                fields: {
    ...
    QUnit.test('simple rendering', async function (assert) {
        ...
        assert.strictEqual(clientAction.$('.o_student_attendance_kiosk_mode h1').text(), 'Employee A',
    ...
    QUnit.test('Attendance Kiosk Mode Test', async function (assert) {
        ...
        await testUtils.mock.addMockEnvironment(clientAction, {
            ...
            mockRPC: function(route, args) {
                if (args.method === 'attendance_scan' && args.model === 'res.partner') {

                    rpcCount++;
                    return Promise.resolve(self.data['res.partner'].records[0]);
                }
    ...
    QUnit.test('Attendance Greeting Message Test', async function (assert) {
        ...
        async function createGreetingMessage (target, barcode){
            var action = {
                ...
                next_action: "de_school_attendance.hr_attendance_action_kiosk_mode",
                barcode: barcode,
            };
            ...
            await testUtils.mock.addMockEnvironment(clientAction, {
                ...
                mockRPC: function(route, args) {
                    if (args.method === 'attendance_scan' && args.model === 'res.partner') {
                        rpcCount++;
                        action.attendance.student_id = [args.args[0], 'Student'];
                        /*
